...
这份文件是本仓库的唯一规则源，供 `Claude Code`、`Codex CLI` 和 `Cursor` 共同复用。

## 目标

- 基于当前仓库变更，为即将执行的提交生成符合仓库约定的 commit message
- 优先判断是否需要拆分为多个原子提交，而不是默认合并为一次提交
...
- 如果建议拆分提交，优先按 `Commit 1`、`Commit 2` 的格式输出，并标明每条对应的文件或领域
